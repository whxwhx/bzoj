...
void init(){
    ...
    rep(i,n + 1, n + m) T[i].mn = T[i].w = i - n, T[i].f = T[i].ch[0] = T[i].ch[1] = null;
    rep(i,1,m){
    ...
    rep(i,1,m){
        rt[i] = rt[i - 1];
        while (j <= m && t[j].first < i) rt[i] = add(rt[i], 1, m, t[j].second), j++;
    }
}
 
int qry(int l, int r){
    return n - qry(rt[l], 1, m, l, r);
}
